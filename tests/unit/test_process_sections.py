...
# モジュールを条件付きでインポート
try:
    from src.process_sections import extract_all_sections, process_section_data, process_all_sections_pure, save_sections_to_csv
except ImportError:
    try:
        from process_sections import extract_all_sections, process_section_data, process_all_sections_pure, save_sections_to_csv
    except ImportError:
        print("警告: process_sectionsモジュールが見つかりません。テストはスキップされます。")
